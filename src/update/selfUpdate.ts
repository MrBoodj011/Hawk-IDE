...
export async function runSelfUpdate(version = 'latest'): Promise<UpdateResult> {
  ...
  const env = {
    ...process.env,
    HAWK_REPO: repo,
    ...(normalizedVersion === 'latest' ? {} : { HAWK_VERSION: normalizedVersion }),
    ...(installDir ? { HAWK_INSTALL_DIR: installDir } : {}),
  };
...
function detectInstallDir(): string | undefined {
  if (process.env.HAWK_INSTALL_DIR) return process.env.HAWK_INSTALL_DIR;
  const exe = basename(process.execPath).toLowerCase();
  if (exe === 'hawk' || exe === 'hawk.exe') return dirname(process.execPath);
  return undefined;
}
 ...
 * host. The script is fetched then executed, so a tampered HAWK_REPO
