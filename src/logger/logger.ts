...
// to ~/.hawk/logs/hawk.log, never to stdout/stderr (the
// TUI owns those), rotates at 4 MB by keeping hawk.log.1.
//
 ...
 * to `~/.hawk/logs/hawk.log`. On any setup error the
...
function defaultLogPath(): string | undefined {
  ...
  return join(home, '.hawk', 'logs', 'hawk.log');
}
